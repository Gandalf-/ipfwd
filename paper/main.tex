...
Tests were run on three different systems and five different network cards.
...
compiled using Netperf. Netperf was chosen because it was readily available on
all test platforms, provides built-in CPU utilization measurement, and a wide
range of test types.

% Explain tests
The test type UDP\_STREAM was chosen over TCP\_STREAM for simplicity and to
preserve CPU cycles for IPFW. Raw network throughput and CPU performance is the
target metric, which can be difficult to assess with TCP. (REFERENCE)

Netperf is single threaded, which allows easier comparison of results between
systems with different numbers of cores. Additionally, all tests were run while
the system was idle to minimize contention over system resources. 
...
Intuitively, larger production rule sets would benefit even further from IPFWD.

CPU utilization measurement is the most difficult parameter to measure in these
tests. There are many factors that can account for variability between tests
and jitter in the results.
...
and NIC throughput bound. We can see that IPFWD affects performance positively
but differently for both types. 
...
tests, which have the weakest CPU but 10Gb Ethernet NICs. This finding is also
apparent in the 40Gb Isilon system.
...
which allows more cycles to be dedicated to other system processes.
